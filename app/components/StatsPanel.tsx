...
export const useSceneStats = () => {
  ...
  const mark = useCallback(() => {
    ...
    if (time >= prevTimeRef.current + 1000) {
      const fps = Math.round((framesRef.current * 1000) / (time - prevTimeRef.current))
      setCurrentFPS(fps)
  ...
  const calculateSceneStats = useCallback((scene: THREE.Scene) => {
    ...
    for (let i = 0, l = scene.children.length; i < l; i++) {
      ...
      object.traverseVisible((object: any) => {
        objects++
  ...
  const StatsPanel = useCallback(
    (props: ViewProps) => {
      return (
        <View {...props}>
          {(sceneStats.objects > 0 || sceneStats.vertices > 0 || sceneStats.triangles > 0) && (
            <>
